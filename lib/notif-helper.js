...
const ACTION = { 'JOIN_WORKSPACE': 'jw', 'ADD_NEW_VERSION': 'aw' };
...
const actionHelper = {
    'jw': async ({ data, workspace: workspaceName }) => {
        try {
            let workspace = await require("../models/workspace-model").findOne({ workspaceName }).select("-__v").exec();
